...
| コンテナ | イメージ | メモリ上限 |
| --- | --- | --- |
| Caddy | caddy:2 | 256 MB |
| GitLab CE | gitlab/gitlab-ce:X.Y.Z-ce.0 | 5 GB |
| Jenkins | jenkins/jenkins:lts | 3 GB |
| Redmine | redmine:X.Y.Z | 1 GB |
| SonarQube | sonarqube:community | 3 GB |
| PostgreSQL（Redmine/SonarQube 共有） | postgres:15 | 1 GB |
...
- Caddyfile は数行で完結（自動 HTTPS が標準機能）
- 証明書は Let's Encrypt（HTTP-01 challenge）で自動取得・更新
- 証明書は Docker volume で永続化
...
- 6.3 節の OS チューニング設定を user_data で適用
- `docker-compose.yml`・`Caddyfile` はリポジトリで管理し、
  Systems Manager 経由または Git pull で配布
- 機密情報（OAuth Client Secret 等）は SSM Parameter Store
  （SecureString）または Secrets Manager で管理し、
  Docker Compose に環境変数として注入
